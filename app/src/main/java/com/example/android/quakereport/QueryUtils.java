...
public final class QueryUtils {
    ...
    public static ArrayList<Earthquake> extractEarthquakes() {
        ...
        try {
            ...
            Date dateObj;
            SimpleDateFormat dateFormat= new SimpleDateFormat("MMM DD,yyyy h:m a", Locale.ENGLISH);

            for (int i = 0; i<features.length();i++){
                JSONObject eartquake = features.getJSONObject(i);
                JSONObject properties = eartquake.getJSONObject("properties");
                double mag= properties.getDouble("mag");
                long time= properties.getLong("time");
                dateObj= new Date(time);
                String place= properties.getString("place");
                String dataToDisplay = dateFormat.format(dateObj);
